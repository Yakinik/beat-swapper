...
音声ファイル
 ...
 └─ OfflineAudioContext(1, n, 44100)   → 解析用 Float32Array（44.1kHz モノラル）
   ...
   slice-plan.ts グリッド + 開始インデックス + 拍数 + 並び順
                 → BeatSlice[] { offset, duration, startAt }
         │
         ▼
...
- Essentia は曲の最初の一撃を落とすことがある。そのまま使うと本当の 1 拍目がグリッド上に
  ...
  が前後 4 拍まで外挿して埋める（既に足りている曲には何も足さない）。
- 小節の頭は「位相 0〜3」ではなく「グリッド上の開始インデックス」で持つ。負も取り、
  `gridTime()` が周辺の拍間隔で外挿する。負のインデックスは音源より手前を指し、
  スケジューラが何もスケジュールしないことで無音になる。
- 1 小節の拍数（3 / 4）は検出済みの拍列の解釈が変わるだけなので、切り替えても解析は
  やり直さない。位相の推定だけ計算し直す。
- BPM か開始位置を手で触ると、検出した拍位置を捨てて等間隔グリッドへ切り替わる
  （`makeUniformTicks()`）。テンポ一定の曲でグリッドごとずれているときの逃げ道。
  テンポが揺れる曲では後半でずれるので、検出モードのほうが向く。
- 波形は元音源のタイムラインで描くが、開始位置が t < 0 のときは表示範囲を負から
  始めて斜線で塗る。鳴っている範囲には薄いアクセントの帯を敷くので、波形が無い
  区間でも再生位置が分かる。拡大側は再生中の小節以外に背景色を重ねて伏せる。
