...
- **つなぎ目のフェードはスライスの内側で完結させる。** 重ねてクロスフェードすると全体の
  長さが縮み、拍の位置が元とずれる。
- **iOS の消音スイッチ対策で `navigator.audioSession.type = 'playback'` を設定する。**
  既定の `'auto'` は Safari では ambient 相当に落ち、Web Audio の音が消音スイッチに
  従って無音になる（`<audio>` 要素は従わないので「他のサイトは鳴るのにここだけ鳴らない」
  という症状になる）。Safari / iOS Safari 16.4 以降のみの API なので存在確認してから触る。
  **設定はモジュール読み込み時ではなく再生操作の中で行う**（`unlockAudioContext()`）。
  `'playback'` は排他カテゴリで他アプリの音楽を止めるため、押す前に奪ってはいけない。
- **iOS はユーザー操作と同じタスクの中で AudioContext を解錠しないと鳴らない。**
  `resume()` の完了を待つあいだに操作の資格が切れることがあるので、`await` より前に
  無音を 1 サンプル鳴らす。`resumeAudioContext()` の判定は `!== 'running'` にする
  （iOS は着信やバックグラウンド化で `'interrupted'` になり、`'suspended'` だけ見ると復帰できない）。
- **再生速度はピッチを保つ。** `playbackRate` を動かすとピッチも動くので、スケジューラの
  中で重ね合わせ（OLA）による時間伸縮をしている。グレインは等速のまま鳴らし、読み出し
  位置だけを速度ぶん進めて半分ずつ重ねる。**グレインの読み出しはスライスの範囲を超えない**
  （はみ出させると隣の拍が混じって並べ替えた意味が薄れる）。等速のときは分割しない。
- スケジューラでは **出力時間（計画上の位置）と実時間（AudioContext の時刻）を混ぜない**。
  実時間 = 出力時間 ÷ 再生速度。`start()` の offset / duration はバッファ自身の時間軸
  なので出力時間のまま渡す。
- 並び順は重複・省略を許すので（`1133` `124`）、出力側の小節長は元と変わりうる。
  各スライスが持つ `startAt`（出力タイムライン上の開始位置）を必ず経由すること。
- **小節の頭は「位相 0〜3」ではなく「グリッド上の開始インデックス」で持つ**
  （`model/analysis.ts` の `startIndex`）。負も取り、`gridTime()` が周辺の拍間隔で
  外挿する。負のインデックスは音源より手前を指し、スケジューラが何もスケジュール
  しないことで無音になる。位相を 0〜3 に閉じ込めると `ticks[0]` より手前を小節の頭に
  できず、曲頭が落ちる。
- **1 小節の拍数は 3 か 4 の可変**（`model/bar-shape.ts`）。検出済みの拍列の解釈が
  変わるだけなので解析はやり直さない。切り替えは `beat-order.ts` の
  `setBeatsPerBar()` を通す（並び順が新しい拍数で無効になったら直す必要がある）。
  拍数を固定値でハードコードしない。
- 再生位置（`playhead`）は一時停止・停止のあいだも保持する。再生中はプレイヤーの
  時計が真、停止中は `playhead` が真。混ぜると位置が飛ぶ。
- **BPM か開始位置を手で触ると、検出した拍位置を捨てて等間隔グリッド
  （`makeUniformTicks()`）へ切り替わる。** BPM を変えることに他の解釈がないため。
  手動グリッドでは特徴量と拍が対応しないので位相の自動推定は使わず、開始位置が
  そのまま 1 拍目になる。どちらの編集も「表示している開始位置を保つ」ように原点を
  逆算する（`anchorStart()`）。`resetDetection()` で検出結果へ戻る。
